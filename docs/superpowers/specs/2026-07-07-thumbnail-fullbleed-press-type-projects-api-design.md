# サムネイルのフルブリード化 + ホバーズーム / Press 種別削除 / works → projects API 統一
...
- `Work` → `Project`、`WorkCategory` → `ProjectCategory`
  （`"project" | "oss" | "research"` の 3 値）、`WorkLink` → `ProjectLink`。
- `LinkKind` は変更なし。

### `src/lib/microcms/`

- `types.ts` — `RawWork` → `RawProject`、`RawWorkLink` → `RawProjectLink`。
...
2. 移行スクリプトを実行。
3. サイト表示を確認後、管理画面で `works` API を削除。
4. 管理画面で press の `type` フィールドを削除。

## テスト・検証

- `mappers.test.ts` の更新でユニットテストが通ること。
- ローカルで dev サーバーを起動し、ホーム（Projects / Press タイル）・/press・/blogs で
  サムネイルがカード縁までフィットし、ホバーでズームすることを確認。
- 移行スクリプト実行後、projects 由来の 10 件がサムネイル込みで表示されることを確認。
